...
class Item {
  ...
  void startFalling() {
    if (stock > 0) {
      stock--;
      if (stock == 0) isEmpty = true; // kurangi stok
      isStuck = true;
      stuckTimer = 0;
      isFalling = false;
      velocityX = random(-0.2, 0.2);
      rotation = 0;
      alpha = 0; // mulai animasi jatuh
    }
  }
  ...
  void updateFalling() {
    if (isStuck) {
      ...
      rotation = sin(stuckTimer * 0.2) * 0.01;
      if (stuckTimer > 120) {
        isStuck = false;
        isFalling = true;
        fallSpeed = 0;
        velocityX = random(-0.4, 0.4);
        displayPos.x = originalX;
      } // lepas stuck
    } else if (isFalling) {
      fallSpeed += 0.021;
      displayPos.y += fallSpeed; // gravitasi
  ...
  void moveTo(int newRow, int newCol) {
    this.row = newRow;
    this.col = newCol;
    this.position = new PVector(newCol, newRow); // pindah grid
  }
  ...
  void slideToPosition(float speed) {
    ...
    displayPos.z = lerp(displayPos.z, target.z, speed); // animasi geser
  }
}
